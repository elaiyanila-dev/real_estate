        ...
        <div className="surface rounded-3xl p-8">
          <div className="mb-7 text-center">
            <motion.div
              ...
              className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-[#F0FAF8] text-[#0F766E]"
            >
              <ShieldCheck size={28} />
            </motion.div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <label className="block text-sm font-bold text-[#1F2937]">
              ...
              <div className="relative mt-2">
                ...
                <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#6B7280] transition-colors duration-200 hover:text-[#0F766E]">{showPass ? <EyeOff size={16} /> : <Eye size={16} />}</button>
              </div>
            </label>
            <motion.button whileTap={{ scale: 0.98 }} className="btn-primary w-full rounded-2xl py-3 font-bold">Admin Sign In</motion.button>
          </form>
        </div>
      </motion.div>
    </div>
