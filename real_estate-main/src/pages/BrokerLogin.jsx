  ...
  return (
    <div className="grid min-h-screen bg-[#FAF9F6] lg:grid-cols-[.9fr_1.1fr]">
      <motion.aside
        ...
        className="hidden bg-[#134E4A] p-12 text-white lg:flex lg:flex-col lg:justify-center"
      >
        <Link to="/" className="mb-14 flex items-center gap-3 transition-opacity duration-200 hover:opacity-90">
      ...
      <main className="flex items-center justify-center px-4 py-10">
        ...
        >
          <Link to="/" className="mb-6 inline-flex items-center gap-2 text-sm font-bold text-[#6B7280] transition-colors duration-200 hover:text-[#0F766E]">
            <ArrowLeft size={16} /> Back to Home
          </Link>
          <div className="surface rounded-3xl p-8">
            <div className="mb-6 grid grid-cols-2 rounded-2xl bg-[#F8F8F7] p-1">
              <button onClick={() => setMode('login')} className={`rounded-xl py-2 text-sm font-bold transition-all duration-200 ${mode === 'login' ? 'bg-white text-[#0F766E] shadow-sm' : 'text-[#6B7280] hover:text-[#1F2937]'}`}>Broker Login</button>
              <button onClick={() => setMode('register')} className={`rounded-xl py-2 text-sm font-bold transition-all duration-200 ${mode === 'register' ? 'bg-white text-[#0F766E] shadow-sm' : 'text-[#6B7280] hover:text-[#1F2937]'}`}>Register</button>
            </div>
            <h1 className="text-3xl font-extrabold text-[#1F2937]">{mode === 'login' ? 'Broker Login' : 'Create Broker Account'}</h1>
            ...
            {mode === 'login' ? (
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                <motion.button whileTap={{ scale: 0.98 }} className="btn-primary w-full rounded-2xl py-3 font-bold">Sign In</motion.button>
              </form>
            ) : (
              <form onSubmit={(event) => { event.preventDefault(); navigate('/dashboard') }} className="space-y-3">
                ...
                <Password value={form.password} onChange={(value) => update('password', value)} show={showPass} setShow={setShowPass} />
                <motion.button whileTap={{ scale: 0.98 }} className="btn-primary w-full rounded-2xl py-3 font-bold">Create Account</motion.button>
              </form>
            )}
            <p className="mt-6 text-center text-sm text-[#6B7280]">
              Customer? <Link to="/user-login" className="font-bold text-[#0F766E] transition-colors duration-200 hover:text-[#134E4A]">Sign in here</Link>
            </p>
          </div>
        </motion.div>
      </main>
  ...
  return (
    <label className="block text-sm font-bold text-[#1F2937]">
      ...
      <div className="relative mt-2">
        ...
        <button type="button" onClick={() => setShow(!show)} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#6B7280] transition-colors duration-200 hover:text-[#0F766E]">{show ? <EyeOff size={16} /> : <Eye size={16} />}</button>
      </div>
    </label>
  )
}
