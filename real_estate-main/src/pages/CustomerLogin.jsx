...
export default function CustomerLogin() {
  ...
  return (
    <AuthShell backLabel="Back to Home" title="User Login" subtitle="Access saved homes, wishlist, and recommendations.">
      <div className="mb-5 grid grid-cols-2 rounded-2xl bg-[#F8F8F7] p-1">
        <button onClick={() => setTab('email')} className={`rounded-xl py-2 text-sm font-bold transition-all duration-200 ${tab === 'email' ? 'bg-white text-[#0F766E] shadow-sm' : 'text-[#6B7280] hover:text-[#1F2937]'}`}>Email</button>
        <button onClick={() => setTab('phone')} className={`rounded-xl py-2 text-sm font-bold transition-all duration-200 ${tab === 'phone' ? 'bg-white text-[#0F766E] shadow-sm' : 'text-[#6B7280] hover:text-[#1F2937]'}`}>Phone OTP</button>
      </div>
      ...
      {tab === 'email' ? (
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <div className="flex items-center justify-between text-sm">
            ...
            <a href="#" className="font-bold text-[#0F766E] transition-colors duration-200 hover:text-[#134E4A]">Forgot password?</a>
          </div>
        ...
        <div className="space-y-4">
          ...
          <motion.button whileTap={{ scale: 0.98 }} className="btn-primary w-full rounded-2xl py-3 font-bold">Send OTP</motion.button>
        </div>
      )}
      <p className="mt-6 text-center text-sm text-[#6B7280]">
        Are you a broker? <Link to="/broker-login" className="font-bold text-[#0F766E] transition-colors duration-200 hover:text-[#134E4A]">Broker Login</Link>
      </p>
    </AuthShell>
  )
}
        ...
        <div className="surface rounded-3xl p-8">
          <div className="mb-7 text-center">
            <motion.div
              ...
              className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-[#0F766E] text-xl font-extrabold text-white shadow-sm"
            >
              A
            </motion.div>
  ...
  return (
    <label className="block text-sm font-bold text-[#1F2937]">
      ...
      <div className="relative mt-2">
        ...
        <button type="button" onClick={() => setShow(!show)} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#6B7280] transition-colors duration-200 hover:text-[#0F766E]">
