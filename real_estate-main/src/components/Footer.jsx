      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-14 sm:px-6 md:grid-cols-[1.3fr_2fr] lg:px-8">
        <div>
          <Link to="/" className="flex items-center gap-3 transition-opacity duration-200 hover:opacity-90">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#0F766E] text-lg font-extrabold text-white shadow-sm">A</div>
            <div>
          ...
          {Object.entries(groups).map(([title, links]) => (
            <div key={title}>
              ...
              <ul className="mt-4 space-y-2">
                {links.map((link) => (
                  <li key={link}>
                    <a href="#" className="text-sm text-[#6B7280] transition-colors duration-200 hover:text-[#0F766E]">{link}</a>
                  </li>
                ))}
              </ul>
            </div>
